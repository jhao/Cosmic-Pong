...
import { playHitSound, playWinSound, playLoseSound, initAudio } from '../utils/audio';
...
const PLAY_AREA_HEIGHT_RATIO = 0.9; // Game happens in top 90%, bottom 10% is for controls
...
const Game: React.FC<GameProps> = ({ level, settings, onGameWin, onQuit }) => {
  ...
  const [gameState, setGameState] = useState<'countdown' | 'serving' | 'playing' | 'point-end' | 'paused'>('countdown');
  const [pointEndInfo, setPointEndInfo] = useState<{ winner: 'player' | 'ai' } | null>(null);
  const [countdown, setCountdown] = useState(3);
  const [servingPlayer, setServingPlayer] = useState<'player' | 'ai'>(Math.random() < 0.5 ? 'player' : 'ai');
  const [startTime, setStartTime] = useState<number | null>(null);
  const isAudioInitialized = useRef(false);

  // Fix: Initialize useRef with null to provide an initial value and satisfy TypeScript's type requirements.
  const animationFrameId = useRef<number | null>(null);

  const resetPositions = useCallback(() => {
    if (!gameAreaRef.current) return;
    const { width, height } = gameAreaRef.current.getBoundingClientRect();
    const gameHeight = height * PLAY_AREA_HEIGHT_RATIO;
    const paddleWidth = width * PADDLE_WIDTH_RATIO;
    const ballRadius = (paddleWidth * BALL_PADDLE_RATIO) / 2;

    setPlayerPaddle({ x: width / 2 - paddleWidth / 2, y: gameHeight - PADDLE_HEIGHT, width: paddleWidth, height: PADDLE_HEIGHT });
    setAiPaddle({ x: width / 2 - paddleWidth / 2, y: PADDLE_HEIGHT, width: paddleWidth, height: PADDLE_HEIGHT });
    
    let ballY = servingPlayer === 'player' ? gameHeight - PADDLE_HEIGHT - ballRadius - 5 : PADDLE_HEIGHT * 2 + ballRadius + 5;
    let ballX = width / 2;
    ...
    setObstacles(level.obstacles.map(obs => ({
        ...obs,
        x: (obs.x / 100) * width,
        y: (obs.y / 100) * gameHeight, // Obstacles are within the play area
    })));
  ...
  useEffect(() => {
      ...
      if (gameState === 'point-end' && pointEndInfo) {
          const pointWinner = pointEndInfo.winner;
          ...
          // Check for game over
          if ((newPlayerScore >= WINNING_SCORE || newAiScore >= WINNING_SCORE) && Math.abs(newPlayerScore - newAiScore) >= 2) {
              // Game finished, wait then call onGameWin/onQuit
              setTimeout(() => {
                  if (newPlayerScore > newAiScore) {
                      const endTime = Date.now();
                      const timeSeconds = startTime ? Math.round((endTime - startTime) / 1000) : 0;
                      onGameWin({ levelId: level.id, playerScore: newPlayerScore, aiScore: newAiScore, timeSeconds });
                  } else {
                      onQuit();
                  }
              }, 2000);
              ...
              setTimeout(() => {
                  setServingPlayer(pointWinner === 'player' ? 'ai' : 'player');
                  setPointEndInfo(null);
                  setGameState('serving');
              }, 2000);
          }
      }
  }, [gameState, pointEndInfo]);
  ...
  const gameLoop = useCallback(() => {
    ...
    setBall(prevBall => {
      ...
      // Paddle collisions
      // Player
      if (newVy > 0 && newY + prevBall.radius > playerPaddle.y && newY - prevBall.radius < playerPaddle.y + playerPaddle.height && newX > playerPaddle.x && newX < playerPaddle.x + playerPaddle.width) {
        playHitSound();
        newVy = -Math.abs(newVy) * elasticity;
        const hitPos = (newX - (playerPaddle.x + playerPaddle.width / 2)) / (playerPaddle.width / 2); // -1 to 1
        newVx += hitPos * 3; // Refined paddle physics
      }
      // AI
      if (newVy < 0 && newY - prevBall.radius < aiPaddle.y + aiPaddle.height && newY + prevBall.radius > aiPaddle.y && newX > aiPaddle.x && newX < aiPaddle.x + aiPaddle.width) {
        playHitSound();
        newVy = Math.abs(newVy) * elasticity;
      }
      ...
      obstacles.forEach(obs => {
          if (obs.type === ObstacleType.Rectangle) {
              ...
              if ((dx * dx + dy * dy) < (prevBall.radius * prevBall.radius)) {
                  ...
                  if (distX > distY) newVx = -newVx; else newVy = -newVy;
              }
          } else { // Circle
              const dist = Math.hypot(newX - (obs.x + obs.width / 2), newY - (obs.y + obs.height/2));
              if (dist < prevBall.radius + obs.width / 2) { newVy = -newVy; }
          }
      });

      // Scoring
      if (newY - prevBall.radius < 0) { // Player scores
        setPointEndInfo({ winner: 'player' });
        setGameState('point-end');
        return prevBall;
      }
      if (newY + prevBall.radius > gameHeight) { // AI scores
        setPointEndInfo({ winner: 'ai' });
        setGameState('point-end');
        return prevBall;
      }
  ...
  const handlePointerMove = (clientX: number) => {
    if (!gameAreaRef.current || gameState === 'paused' || gameState === 'countdown') return;
    const { left, width } = gameAreaRef.current.getBoundingClientRect();
    const newX = clientX - left - playerPaddle.width / 2;
  ...
  return (
    <div className="w-full h-full max-w-lg max-h-[90vh] aspect-[9/16] bg-stone-900 border-4 border-cyan-700 relative shadow-2xl shadow-cyan-900/50 flex flex-col cursor-pointer" onMouseMove={handleMouseMove} onTouchMove={handleTouchMove} onClick={handleServe}>
      <div className="relative flex-grow" ref={gameAreaRef}>
        {/* Score Watermark */}
        <div className="absolute inset-0 flex items-center justify-center text-[20vh] font-black text-cyan-900/30 select-none z-0">
          {aiScore} - {playerScore}
        </div>
        ...
        {/* Game State Overlay */}
        {gameState === 'countdown' && (
          <div className="absolute inset-0 bg-black/50 flex items-center justify-center text-8xl font-bold text-white z-20">
            {countdown > 0 ? countdown : 'Go!'}
          </div>
        )}
        {gameState === 'serving' && (
          <div className="absolute inset-0 bg-black/30 flex flex-col items-center justify-center text-2xl font-bold text-white animate-pulse z-20">
              <p>{servingPlayer === 'player' ? 'Your Serve' : 'Opponent Serves'}</p>
              <p className="text-lg mt-2">Tap to Start</p>
          </div>
        )}
        {gameState === 'point-end' && pointEndInfo?.winner === 'player' && <Fireworks />}
        {gameState === 'point-end' && pointEndInfo?.winner === 'ai' && (
             <div className="absolute inset-0 bg-black/50 flex items-center justify-center text-2xl font-bold text-white z-20">
                再接再厉不要气馁
             </div>
        )}
      </div>
       <div className="h-[10%] bg-stone-900/50 border-t-2 border-cyan-800 flex items-center justify-center">
            <p className="text-stone-600 text-sm">Control Area</p>
       </div>
      <button onClick={onQuit} className="absolute top-2 right-2 px-3 py-1 text-xs bg-red-800/80 text-white rounded z-30">Quit</button>
    </div>
  );
};
